...
<body>
  ...
  <script>
    ...
    function voteNews(url, type) {
      ...
      // 更新显示
      const upSpan = document.querySelector();
      const downSpan = document.querySelector();
...
def fetch_news_with_skill():
    ...
    for attempt in range(max_retries):
        try:
            ...
            if result.returncode == 0:
                ...
                if json_start >= 0:
                    ...
                    if isinstance(data, list):
                        news = dedup_news(data)
                        if news:  # If we got news, return it
                            return news
                        # If we got empty list, it might be rate limiting or genuinely no news
                        if attempt < max_retries - 1:
                            delay = base_delay * (2 ** attempt) + random.uniform(0, 1)
                            print(f"⚠️ 技能获取返回空数据，{delay:.1f}秒后重试... (尝试 {attempt+1}/{max_retries})")
                            time.sleep(delay)
                            continue
                        else:
                            return []
                    elif isinstance(data, dict) and "items" in data:
                        news = dedup_news(data["items"])
                        if news:
                            return news
                        if attempt < max_retries - 1:
                            delay = base_delay * (2 ** attempt) + random.uniform(0, 1)
                ...
                # Non-zero return code might indicate an error
                if attempt < max_retries - 1:
                    delay = base_delay * (2 ** attempt) + random.uniform(0, 1)
                    print(f"⚠️ 技能执行失败 (返回码: {result.returncode})，{delay:.1f}秒后重试... (尝试 {attempt+1}/{max_retries})")
                    time.sleep(delay)
                    continue
                else:
                    print(f"❌ 技能执行失败: {result.stderr}")
                    return []
        except subprocess.TimeoutExpired:
            if attempt < max_retries - 1:
                delay = base_delay * (2 ** attempt) + random.uniform(0, 1)
                print(f"⚠️ 技能执行超时，{delay:.1f}秒后重试... (尝试 {attempt+1}/{max_retries})")
                time.sleep(delay)
                continue
            else:
                print(f"❌ 技能执行超时")
            ...
            if any(keyword in error_str for keyword in ['429', 'rate limit', 'too many requests', 'quota exceeded']):
                if attempt < max_retries - 1:
                    delay = base_delay * (2 ** attempt) + random.uniform(0, 2)
                    print(f"⚠️ 检测到API限流: {e}")
                    print(f"   等待{delay:.1f}秒后重试... (尝试 {attempt+1}/{max_retries})")
                    time.sleep(delay)
                    continue
